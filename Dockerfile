...
RUN apt-get update && apt-get install -y build-essential curl zlib
...
RUN apt-get install -y git libssl-dev libreadline-dev nodejs npm postgresql-client postgresql postgresql-contrib libpq-dev
RUN apt-get clean
RUN npm -g install n
RUN n latest
